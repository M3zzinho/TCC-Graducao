...
An anxious mind would immediately panic. We have a \textsc{to-do} list before calling it a day:
\begin{itroman}
...
\begin{definicao}
    Using construction \ref{contru: index map}, the application
    \begin{equation*}
        \function{{\partial}{K_1(A/I)}{K_0(I)}{[x]_1}{[wp_n\inv w]_0 - [p_n]_0}}
    \end{equation*}
    whenever $x\in \GL_n(\widetilde{A/I})$ and $w$ is a lift of $\diag(x,\inv x)$ is the group morphism so called \textit{index} map.
\end{definicao}
