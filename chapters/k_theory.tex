...
While $K_0$ is build upon equivalence classes of idempotent, $K_1$ uses invertible elements, and this makes the construction simpler. Let $\GL_\infty(A) \coloneqq \varinjlim_{n\in\N} \GL_n(A)$ considering the embedding $x \longmapsto \diag(x, 1)$. Calculus is back, and we shall consider exponentials inside a unital algebra $A$:
...
\begin{exemplo}
    ...
    \end{eqnarray*}
    Notice that when $A = \mathscr B(H)$ and $I = \mathscr K(H)$, we dealing again with Fredholm operators living in the Calkin algebra $\mathscr Q(H)$ and $\partial$ coincides with the Fredholm index. 
\end{exemplo}