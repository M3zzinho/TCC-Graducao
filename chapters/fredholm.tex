...
\begin{quote}
    ...
    \end{observacao}
\end{quote}
With this digression in mind, notice that if $(\sub\Id E - TS) \in \mathscr K(E,F)$, there exist an element $ S$ such that both $\sub\Id E - TS$ and $\sub\Id F-ST$ are finite-rank operators, i.e., $T$ is Fredholm.
\end{proof}
\end{proposicao}
...
\begin{equation*}
    \ind(T_1T_2) = \ind(H_{\pi/2}) = \ind(H_0) = \ind(T_1 \oplus 
    )= \ind T_2 + \ind T_1.
\end{equation*}
...
\begin{equation*}
    \con\ind T \coloneqq [\ker p_n\circ T]_{0} - [X \times H^\perp_n]_0
\end{equation*}
