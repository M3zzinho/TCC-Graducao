...
\begin{lema}\label{lema: [P]_0 = [Q]_0 ---> [p]_0 = [q]_0}
    ...
    \begin{proof}
        ...
        We know that $p=\left(\inner{x_i}{y_j}\right)_{i,j}$ and $q=\left(\inner{z_i}{w_j}\right)_{i,j}$ by the argument used in . Therefore:
...
Notice that we can consider the index map $\partial$ induced by (\ref{eq:seq exata de C-star em K(H_A)}). Since $\sub{T}{\mathscr H_A\mathscr H_A}$ is a compact perturbation of the identity and both $M$ and $N$ are finite rank modules, one concludes that $F$ is also a compact perturbation of the identity. Therefore, $\pi(F)= \sub1{\mathscr Q(\mathscr H_A)}$. Since $\Id_2$ is a lift of $\diag(\sub1{\mathscr Q(\mathscr H_A)}, \inv{\sub1{\mathscr Q(\mathscr H_A)}})$, one can obtain that $\partial([\sub1{\mathscr Q(\mathscr H_A)}]_1) = [\Id_2 p_2\Id_2^{-1}]_0 - [p_2]_0 = 0$.
