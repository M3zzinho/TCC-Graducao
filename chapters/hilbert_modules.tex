...
\begin{proposicao}[Equivallence of positivity]
...
Since $\|p_{n} y\|$ tends to zero, the same is true for the norm of the restriction of the operator $\Omega_{x}^{}\Omega_{y}^{*}$ to the submodule $L_{n}(A)^{\perp}$, hence, for the norm of any compact operator $K$.
    ...
    \begin{equation*}
        ...
        \end{array}
    \end{equation*}
    If $z \in L_n(A)$, one can see that $Kz = \Omega_{Ke^n}^{\vphantom{*}}\Omega_{e^n}^*(z)$, so that the supreme can be taken in the hole $\mathscr H_A$. Therefore:
    \begin{equation*}
...
\begin{itroman}
    ...
    \item If $A$ is a unital algebra, $\mathscr K(A) = \adj(A)$ since any adjointable operator $T$ consists of left multiplication by $T(1)$.
...
\begin{eqspaced*}{}
    \begin{array}{rcl}
        T\Omega_y\Omega_x^* + \Omega_w\Omega_z^*S &=& \sum\limits_{i=1}^n   Ty_i\inner{x_i}{\,\cdot\,}  + \sum\limits_{i=1}^n w_i\inner{S^*z_i}{\,\cdot\,} \\
...
\section{Rank definition of Finite Rank Hilbert Modules}
...
\begin{lema}\label{lema: [P]_0 = [Q]_0 ---> [p]_0 = [q]_0}
    Let $E$ be a Hilbert module and $P, Q \in \adj(E)$ be two idempotent operators. If $P$ and $Q$ are similar, i.e., there exists $u \in \GL \adj(E)$ such that $P = uQ\inv u$, the matrices $p$ and $q$ are Murray-von Neumann equivalent. 
    \begin{proof}
        ...
        We know that $p=\left(\inner{x_i}{y_j}\right)_{i,j}$ and $q=\left(\inner{z_i}{w_j}\right)_{i,j}$. Therefore:

        $$
        \Omega_y\Omega_x^* = P = uQ\inv u = u\Omega_{w}\Omega_z^*\inv u = \Omega_{u(w)}^{\,}\Omega_{({\inv u})^*(z)}^* $$
        $$\Rightarrow \sum_{i=1}^n y_i \inner{x_i}{\come} = \sum_{i=1}^m u(w_i)\inner{(\inv u)^*(z_i)}{\come}$$ 
        Notice that the matrix $\widehat q \coloneqq \left(\inner{(\inv u)^*(z_i)}{u(y_j)}\right)_{i,j}$ obeys $\Im uQ\inv u \simeq \widehat{q}A^s$ by the construction of \ref{teo: M rank finito sse M = pA^n}. But since $\widehat{q} = q$, we obtain two isomorphisms such that the following diagram commutes.
        \begin{equation*}
\begin{tikzcd}
    \Im P \arrow[d]        & \Im uQu^{-1} \arrow[l, no head, equal] \arrow[d] \arrow[r, dashed] & \Im Q \arrow[d] \\
    pA^r \arrow[r, dashed] & qA^s \arrow[r, no head, equal]                                     & qA^s           
    \end{tikzcd}
        \end{equation*}
...
Before lighting that relationship, first we enrich the definition on the special case of finite-rank modules.
...
\begin{equation*}
    \begin{array}{rcl}
        \sub{\mathscr C}0 &\coloneqq& {\big\{ r\pi_i(y) \mid r\in \{\sub{T}{X M}^{\vphantom{*}}, \sub{S}{X N}^{\vphantom{*}}, \sub{T}{N X}^{*},\sub{S}{XM}^{*}\}, \sub\Id{\operatorname{dom}(r)} = \Omega_y^{\,}\Omega_x^*\big\}}_{i\in \mathbb N} \\ 
...
\begin{array}{rcl}
    \mathscr C_{n+1} &\coloneqq & \mathscr C_{n} \cup \sub{T}{X X}^{\vphantom{*}}\left(\mathscr C_{n}\right) \cup \sub{S}{X X}^{\vphantom{*}}\left(\mathscr C_{n}\right) \\
    & & \cup \sub{T}{X X}^{*}\left(\mathscr C_{n}\right) \cup \sub{S}{X X}^{*}\left(\mathscr C_{n}\right)
\end{array}
\end{eqspaced*}
...
\begin{tikzcd}
0 \arrow[r] & \mathscr K(\mathscr H_A) \arrow[r, hook] & \adj(\mathscr H_A) \arrow[r, "\pi", two heads] & \mathscr Q(\mathscr H_A) \arrow[r] & 0
\end{tikzcd}
\end{equation}

Notice that we can consider the index map $\partial$ induced by (\ref{eq:seq exata de C-star em K(H_A)}). Since $\sub{T}{\mathscr H_A\mathscr H_A}$ is a compact perturbation of the identity and both $M$ and $N$ are finite rank modules, one concludes that $F$ is also a compact perturbation of the identity. Therefore, $\partial([\pi(F)]_1)=\partial([\sub\Id{\mathscr Q(\mathscr H_A)}]_1) = 0$.

We can extract more information by writing down who is the index in terms of $F$ and $G$. By \ref{teo item: I-FG e I-GF sao compactos}, is easy to see that $\pi(F)$ and $\pi(G)$ are each others inverse in $\mathscr Q(\mathscr H_A)$. In order to compute the index, the element  
\begin{equation*}
    w \coloneqq \begin{pmatrix}
        F & \Id - FG \\ \Id-GF & G
    \end{pmatrix} \in \GL_2^{(0)}({\mathscr Q(\mathscr H_A)})
\end{equation*}
is a lift of $\diag(\pi(F), \pi(F)^{-1})$, which it's inverse just swaps $F$ and $G$ places. Therefore:
\begin{equation*}
    \label{eq: index pi(F)}
 \begin{array}{rcl}
